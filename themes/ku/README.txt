...
KU boasts a clean HTML5 structure with extensible CSS classes and IDs for
...
KU's goal is to provide themers the building blocks needed to get their
designs up and running quickly and simply.

KU is perfect if you want a simple, smart, and flexible theme starter.
...
KU utilizes a Sass framework for adaptive grids and layouts and general structure of the
...
KU is meant to be YOUR theme. Follow one of the two methods below and you can
rename 'ku' to another name like 'mytheme'. You're also welcome to keep
'ku'.

1. DRUSH: The provided drush install script will duplicate the ku theme
   folder and rename all appropriate files and content to the new theme name you
   provide.

  - Download and enable the ku theme: $ drush en ku
  - Set the theme as default $ drush config-set system.theme default ku
    or copy the includes/ku.drush.inc into your .drush folder.
  - Run the provided drush install script: $ drush ku-install
  ...
  - At this time, you can uninstall ku: $drush pmu ku
...
2. MANUAL: To manually change the name of the theme follow these steps BEFORE
  ...
  - Rename ku.info.yml to mytheme.info.yml
  - edit ku.info.yml and change the name, description, project (can be
    deleted), and change all other instances of 'ku' to 'mytheme'
  - Rename ku.libraries.yml to mytheme.libraries.yml
  - Rename ku.theme to mytheme.theme
  - in ku.theme, change each instance of 'ku' to 'mytheme'
  - Rename config/schema/ku.schema.yml to mytheme.schema.yml
  - Rename each file in config/install from block.block.ku_tools.yml (for
    example) to block.block.mytheme_tools.yml
  - Every file in config/install, change each instance of 'ku' to
    'mytheme'
  - In js/source/scripts.js, change each instance of 'ku' to 'mytheme'
  - In theme-settings.php, change each instance of 'ku' to 'mytheme'
  - In templates/html.html.twig, change each instance of 'ku' to 'mytheme'
  - In templates/menu-local-tasks.html.twig, change each instance of 'ku' to
    'mytheme'
  - In templates/status-messages.html.twig, change each instance of 'ku' to
    'mytheme'

  When renaming, remember the following:
  - Do not simply replace every instance of 'ku' in every file in the theme.
    Most of KU's dependencies use the word 'ku' somewhere and renaming
    these instances will cause KU to break in unpredictable ways.
  ...
  - If you don't bother renaming KU in the above locations, be advised that
    you will run into conflicts with other versions of KU on your site. If
    your site uses more than one theme based on KU, make sure at least one of
    the themes has been renamed properly!


============================
How to compile Sass in KU
============================

To use Sass and automatically compile it within your theme, please refer to "How
to Use Gulp with KU" in the documentation below.
...
- build/scripts.js & source/scripts.js
  ...
  folder. See comments in ku.libraries.yml file to enable the starter
  scripts.js file.
...
The layout used in KU is fairly similar to the Holy Grail method. It has been
...
This is how the page template is buit in KU, and it works in fluid and fixed
layout. Refer to the notes in layout.sass to see how to modify the layout.


===========================
How to Use Gulp with KU
===========================
...
Once Node.JS is installed, go to the root folder of KU and install your Gulp
packages:

  npm install

This will install the neccessary node_modules to run Gulp, including Gulp itself.
Once installed, cd to the root folder of KU and run Gulp via the command
line:
...
Once you start using KU, you will massively change it until you reach the
point where it has nothing to do with KU anymore. Unlike Zen, KU is not
...
version of KU comes out. Always see KU as a STARTER, and as soon as you
start using it, it is not KU anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when KU
has a new version by the update module, simply delete "project = "ku" in
ku.info.yml.
...
Thanks for using KU, and remember to use the issue queue in drupal.org for
any questions or bug reports:

http://drupal.org/project/issues/ku
